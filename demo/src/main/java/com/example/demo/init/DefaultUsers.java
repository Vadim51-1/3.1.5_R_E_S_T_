package com.example.demo.init;

import com.example.demo.model.Role;
import com.example.demo.model.User;
import com.example.demo.repository.RoleRepository;
import com.example.demo.repository.UserRepository;
import com.example.demo.service.RoleService;
import org.springframework.security.crypto.password.PasswordEncoder;
import org.springframework.stereotype.Component;

import javax.annotation.PostConstruct;
import java.util.Set;


@Component
public class DefaultUsers {

    private final UserRepository userRepository;

    private final RoleRepository roleRepository;

    private final PasswordEncoder passwordEncoder;

    private final RoleService roleService;

    public DefaultUsers(UserRepository userRepository, RoleRepository roleRepository, PasswordEncoder passwordEncoder, RoleService roleService) {
        this.userRepository = userRepository;
        this.roleRepository = roleRepository;
        this.passwordEncoder = passwordEncoder;
        this.roleService = roleService;
    }

    @PostConstruct
    public void creationOfDefaultUsers() {
        Role role1 = new Role();
        role1.setRoleName("ROLE_ADMIN");
        role1.setId(1);
        roleService.addRole(role1);

        Role role2 = new Role();
        role2.setRoleName("ROLE_USER");
        role2.setId(2);
        roleService.addRole(role2);


        createUser("Admin@yandex.ru", passwordEncoder.encode("hochyEst"), "Admin", "Admin", 21, Set.of(role1, role2));
        createUser("User@yandex.ru", passwordEncoder.encode("hochySpat"), "User", "User", 54, Set.of(role2));
    }

    private void createUser(String username, String password, String firstName, String lastName, int age, Set<Role> role) {
        User user = new User(username, password, firstName, lastName, age, role);
        userRepository.save(user);
    }
}
